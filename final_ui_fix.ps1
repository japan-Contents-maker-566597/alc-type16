# ============================================================
# Final UI/UX Fix & Entity transition (Fixed Syntax)
# ============================================================
...
foreach ($f in $files) {
    ...
    # 1. Clean up potential Mojibake from previous runs
    $content = $content -replace "縲仙・蠑上€鷹・繧ｿ繧､繝苓ｨｺ譁ｭ", $logo_entity
    $content = $content -replace "繧ゅ≧荳€蠎ｦ險ｺ譁ｭ縺吶ｋ", $retry_entity

    # 2. Re-apply the logic correctly if it's missing (failsafe)
    if ($content -match '<h1>(?!<a)') {
        $relPath = "index.html"
        if ($f.Directory.Name -eq "types") { $relPath = "../index.html" }
        elseif ($f.FullName -match "en\\types") { $relPath = "../index.html" }
        
        $content = [Regex]::Replace($content, '<h1>((?!<a).*?)</h1>', "<h1><a href=""$relPath"">`$1</a></h1>")
    }
...
Write-Host "Mojibake Fix & Entity transition complete."
